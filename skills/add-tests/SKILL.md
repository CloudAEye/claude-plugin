---
name: add-tests
description: Have CloudAEye write unit tests for the uncovered code in an open pull request and post them on the pull request as one-click review suggestions, new test files included. It writes on the PR; it never edits your working tree.
when_to_use: Use when the user asks for tests for a pull request, or to fix the unit test coverage a /cloudaeye:check-pr run reported. Needs an open pull request — there is no working-tree mode.
argument-hint: "<PR number, e.g. 405 or #405>"
allowed-tools: ["mcp__plugin_cloudaeye_cloudaeye__initialize_repository", "mcp__cloudaeye__initialize_repository", "mcp__plugin_cloudaeye_cloudaeye__add_tests", "mcp__cloudaeye__add_tests"]
---

## What this is

CloudAEye reads the repository's call graph to find changed code that no test covers, writes tests for it, and posts them **onto the pull request** as review suggestions — whole new test files included, which is why some rows come back marked as new.

It needs the call graph to know what is already covered, which is why it runs against a pull request rather than your working tree.
...
never the generated test suggestions. That is deliberate: the text is on the pull request,
...
2. **Confirm before running.** Say what will happen in one line — CloudAEye will
   post test suggestions on `<repo>#<number>`, visible to everyone on the pull request —
...
3. Call `mcp__plugin_cloudaeye_cloudaeye__add_tests` with:
   - `repo`: `INIT.repo_full`
...
- **The tests are proposals, not verified passes.** Nothing here ran them. If the user applies the suggestions, running the suite is a separate step and worth saying so.
- **Do not write the tests yourself instead.** If the user wants tests in the working tree rather than on the pull request, say so plainly and offer to write them as an ordinary edit.
